...
(function () {
  ...
  function init() {
    ...
    // Calculate the aspect ratio and set the pixel ratio
    const aspectRatio = window.innerWidth / window.innerHeight;
    renderer = new THREE.WebGLRenderer({ canvas, antialias: true })
    renderer.setPixelRatio(aspectRatio + 0.2); // Set the pixel ratio

    // Set the initial size of the renderer based on 30% of the viewport
    const viewportWidth = window.innerWidth * 0.3;
    ...
    let material = new THREE.MeshBasicMaterial({ color: 0x2E4374  }); // 0xf2ce2e
